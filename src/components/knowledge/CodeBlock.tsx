  ...
  return (
    <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="flex items-center space-x-3">
          <Badge className={`${getLanguageColor(example.language)} border font-medium`}>
            <span className="flex items-center space-x-1">
              ...
              <span className="capitalize">{example.language}</span>
            </span>
          </Badge>
        ...
        <div className="flex items-center space-x-2">
          {canRun && (
            <Button 
              variant="ghost" 
              size="sm" 
              className="h-8 px-3 text-xs hover:bg-green-50 hover:text-green-700 border border-transparent hover:border-green-200"
              onClick={handleRun}
              isLoading={isRunning}
          ...
          <Button 
            ...
            size="sm" 
            onClick={handleCopy}
            className="h-8 px-3 text-xs hover:bg-blue-50 hover:text-blue-700 border border-transparent hover:border-blue-200"
            title="Copy code"
          >
            {copied ? (
              <>
      ...
      <div className="relative">
        <pre className="p-4 bg-gray-900 text-gray-100 text-sm overflow-x-auto leading-relaxed font-mono">
      ...
      {example.explanation && (
        <div className="p-4 bg-blue-50 border-t border-blue-100">
          <div className="flex items-start space-x-2">
            ...
            <div>
              <h6 className="text-sm font-semibold text-blue-900 mb-1">Explanation</h6>
              <p className="text-sm text-blue-800 leading-relaxed">{example.explanation}</p>
            </div>
          </div>
        </div>
      )}
